...
class Gobang(UtilGobang):
    ...
    def sample_noise(self) -> Union[Tuple[int, int, int], None]:
        ...
        if self.action_space:
            ...
            pos = random.choice(self.action_space)
            x, y = pos
            self.action_space.remove(pos)
            # END_YOUR_CODE
            return 2, x, y
        else:
            return None
    ...
    def get_connection_and_reward(self, action: Tuple[int, int, int],
        ...
        reward = (black_2 - white_2) - (black_1 - white_1)
    ...
    def sample_action_and_noise(self, eps: float) -> Tuple[Tuple[int, int, int], Tuple[int, int, int]]:
        ...
        *** IMPORTANT: Remember to remove the selected position (x, y) from self.action_space once the action (1, x, y)
        ...
        if not self.action_space:  # 如果没有可用动作
            return None, None
        
        # 决定是随机探索还是贪心选择
        if random.random() < eps or state not in self.Q:
            # 随机探索
            pos = random.choice(self.action_space)
            x, y = pos
            action = (1, x, y)
            ...
            for pos in self.action_space:
                x, y = pos
                act = (1, x, y)
                if act in self.Q[state] and self.Q[state][act] > best_q:
                    best_q = self.Q[state][act]
                    best_action = act
    ...
    def q_learning_update(self, s0_: np.array, action: Tuple[int, int, int], s1_: np.array, reward: float,
        ...
        # 更新公式
        self.Q[s0][action] = self.Q[s0][action] + alpha * (reward + self.gamma * max_q_next - self.Q[s0][action])
        # END_YOUR_CODE
